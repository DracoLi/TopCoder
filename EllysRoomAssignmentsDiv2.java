...
public class EllysRoomAssignmentsDiv2
{
	public double getProbability(String[] ratings)
	{
		...
		String[] s = rat.split(" ");
		int[] rats = new int[s.length];
		for (int i = 0; i < s.length; i++)
			rats[i] = Integer.valueOf(s[i]);
		
		
		int elly = rats[0];
		Arrays.sort(rats);
		for (int i = 0; i < rats.length / 2; i++) {
			int temp = rats[i];
			rats[i] = rats[rats.length - 1 - i];
			rats[rats.length - 1 - i] = temp;
		}
		
		int r = rats.length % 20 == 0 ? rats.length / 20 : rats.length / 20 + 1;
		int ei = 0;
